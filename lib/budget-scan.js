...
export async function budgetScan({ departDate, returnDate, tripType, adults, budget, flexible, mode = 'flights', semanticQuery = null }) {
  ...
  // Concurrency: env-configurable, capped 1–20. Default 10.
  // >10 risks Google rate-limit (HTML response instead of flights data).
  const CONCURRENCY = Math.min(Math.max(parseInt(process.env.SCAN_CONCURRENCY || '10', 10), 1), 20);
  const allResults = [];
  let idx = 0;
  const workers = Array.from({ length: CONCURRENCY }, async () => {
    while (idx < tasks.length) {
      ...
      // Jitter between tasks — prevents burst against Google even at cached speed
